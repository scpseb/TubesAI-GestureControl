...
// ============ PIR CONFIGURATION ============
#define PIR_ENABLED         true   // Set ke false jika tidak menggunakan PIR (inference akan selalu jalan)
#define PIR_INVERTED        false
#define PIR_USE_PULLDOWN    false  // Set true jika PIR floating
...
void setup()
{
    ...
    Serial.println("===========================================");
    Serial.printf("PIR Enabled: %s\n", PIR_ENABLED ? "YES" : "NO (always inferencing)");
    Serial.printf("PIR Pin: %d\n", PIR_PIN);
    Serial.printf("PIR Inverted: %s\n", PIR_INVERTED ? "YES" : "NO");
...
void pirTask(void *pvParameters)
{
    ...
    Serial.printf("[PIR Task] Using PIN: %d, Enabled: %d, Inverted: %d\n", 
                  PIR_PIN, PIR_ENABLED, PIR_INVERTED);
    
    // Jika PIR disabled, langsung set motion_active = true dan exit task
    if (!PIR_ENABLED) {
        Serial.println("[PIR Task] PIR DISABLED - Inference always running");
        motion_active = true;
    ...
    // Test baca PIR beberapa kali saat startup
    Serial.println("[PIR Task] Initial PIR readings (5 samples):");
    for (int i = 0; i < 5; i++) {
        int val = digitalRead(PIR_PIN);
        Serial.printf("  Read %d: %d (%s)\n", i+1, val, val == HIGH ? "HIGH" : "LOW");
    ...
    while (1) {
        ...
        bool pir_high;
        if (PIR_INVERTED) {
            pir_high = (pirRaw == LOW);   // Inverted: LOW = motion
        } else {
            pir_high = (pirRaw == HIGH);  // Normal: HIGH = motion
        }
        
        // Debug: print saat state berubah
        if (pir_high != lastPirState) {
            lastPirState = pir_high;
            Serial.printf("[PIR Task] State changed: %s (raw=%d)\n", 
                          pir_high ? "MOTION DETECTED" : "NO MOTION", pirRaw);
        }
...
void inferenceTask(void *pvParameters)
{
    ...
    while (1) {
        ...
        // Inference interval (lebih cepat: 150ms = ~6-7 FPS)
